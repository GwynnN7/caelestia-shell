...
bool SessionManager::exec(const QStringList& command) {
    ...
    // Normalise command for the internal D-Bus map
    QString normalizedCmd = cmd.remove("-").remove("_").toLower();

    // 1. Try internal D-Bus methods first
    const auto methodPtr = cmds.value(normalizedCmd, nullptr);
    if (methodPtr) {
        (this->*methodPtr)();
        return true;
    }
    ...
    qCInfo(lcSessionManager) << "Command not in internal map, executing via QProcess:" << command.join(' ');
    
    QString program = command.first();
    QStringList arguments = command.mid(1); // Grabs everything after the first item
    
    // startDetached runs it in the background so it doesn't freeze the UI
    bool success = QProcess::startDetached(program, arguments);
    
    if (!success) {
        qCWarning(lcSessionManager) << "Failed to start external command:" << program;
    }
