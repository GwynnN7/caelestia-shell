                        ...
                        onClicked: mouse => {
                            if (mouse.button === Qt.LeftButton) {
                                ...
                                if (modelData?.toplevels.length > 0) {
                                    ...
                                    const subCmd = modelData?.entry.runInTerminal ? [...GlobalConfig.general.apps.terminal, `${Quickshell.shellDir}/scripts/wrap_term_launch.sh`, ...modelData?.entry.command] : modelData?.entry.command;
